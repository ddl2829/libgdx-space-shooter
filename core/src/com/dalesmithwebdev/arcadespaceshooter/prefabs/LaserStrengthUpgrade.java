...
public class LaserStrengthUpgrade extends Entity {
    public LaserStrengthUpgrade(int level, float xPos, float yPos) {
        if(level == 1) {
            this.add(new RenderComponent(new Texture(Gdx.files.internal("power-ups/powerupGreen_bolt.png"))));
        }
        if(level == 2) {
            this.add(new RenderComponent(new Texture(Gdx.files.internal("power-ups/powerupBlue_bolt.png"))));
        }
